...
function Profilee (props){
    ...
    const saveHandle = async(nama, email, pekerjaan,project,status) => {
     ...
     setData(prevState=>
                [...prevState,
                { "id": data.length + 1, "nama": nama, "email": email, "pekerjaan": pekerjaan, "project":project, "status":status }
                ])
    setCek(true)
    }
            ...
            <CardHeader style={{ backgroundColor: '#47CDF1' }}><h1>Profile</h1></CardHeader>
                ...
                <div style={{ float: 'right' }}>
                    <Button
                        ...
                        onClick={() => { saveHandle(props.nama, props.email, props.pekerjaan, props.project, props.status) }}
                    >
                        Save
                    </Button>
                </div>
